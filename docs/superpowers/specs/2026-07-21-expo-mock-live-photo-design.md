...
- `autoPlay?: boolean`：资源首次准备完成后是否自动播放，默认 `true`。
...
1. 显示封面并加载图片与视频。
2. 两个资源均准备完成后触发 `onLoad`；`autoPlay` 为 `true` 时自动播放一次，为 `false` 时保持封面并等待点击。
...
自动播放按资源版本计算，每个版本仅执行一次。`source` 使用现有结构值判定，`videoSource` 使用 `uri` 判定；任一值变化都创建新版本，因此 A→B→A 是三个版本。版本创建时快照 `autoPlay`，之后即使仍在 loading，单独修改 `autoPlay` 也不启动、暂停或重置当前版本；新值只供下一资源版本使用。
...
- `muted` 开关；
- `autoPlay` 开关；
- Example 在切换 `autoPlay` 时用新的组件 `key` 重建演示实例，让开关效果可以立即观察；这是 Example 行为，不改变库的动态属性语义。
...
- Bun 单元测试覆盖 JS 图片/视频就绪协调、`autoPlay` 开关、默认 Props、图片覆盖层状态和错误转发不抛异常。
...
- 使用 Example 手动验证自动首播开关、结束回封面、点击暂停/恢复和静音切换。

## 文档与非目标

README 只包含安装方式、bare React Native 的 Expo Modules 前置条件、Props、基本示例和 Example 运行命令。

README 必须明确 `autoPlay` 默认 `true`、设为 `false` 时双资源 ready 后停留封面等待点击、每个资源版本最多自动播放一次，以及单独更新属性只影响下一资源版本。
